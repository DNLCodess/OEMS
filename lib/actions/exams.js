...
export async function generateLabCode(examId) {
  const user     = await requireRole('lecturer')
  const supabase = await createClient()
  ...
  while (attempts < 5) {
    code = randomLabCode()
    const { data: existing } = await supabase
      .from('exams').select('id').eq('lab_code', code).maybeSingle()
    if (!existing) break
    attempts++
  }
  ...
  const { error } = await supabase
    .from('exams')
    .update({ lab_code: code, exam_mode: 'lab' })
    .eq('id', examId)

  if (error) return { error: 'Failed to generate lab code.' }

  revalidateExam(examId)
  return { lab_code: code }
}
