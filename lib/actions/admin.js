...
// standing credential — anyone who knows the convention (it's spelled out
// right in the invite UI) could sign in to any invited-but-not-yet-reset
// account. Generate a fresh one per invite instead; it's shown once to the
// admin who created the account so they can relay it to the invitee.
...
// Subdomains that would collide with real static routes in this app and
// make the resulting /{subdomain}/login link permanently broken (shadowed
// by the static route, so it would never even reach the [slug] page).
const RESERVED_SUBDOMAINS = new Set([
  'lab', 'check-result', 'login', 'forgot-password', 'update-password',
  'admin', 'lecturer', 'super-admin', 'dev', 'api',
])

const universitySchema = z.object({
  name:      z.string().min(3, 'University name required'),
  subdomain: z.string().min(2, 'Subdomain required')
    .regex(/^[a-z0-9-]+$/, 'Only lowercase letters, numbers, hyphens')
    .refine(s => !RESERVED_SUBDOMAINS.has(s), 'This subdomain is reserved and cannot be used'),
  primary_color: z.string()
    ...
    .optional()
    .or(z.literal('')),
  // Accepts a full URL (an externally hosted image) or a root-relative path
  // (an asset already shipped in this app's own /public folder, e.g.
  // /pcu/pcu-logo.jpeg) — logos don't have to be hosted elsewhere.
  logo_url: z.string()
...
const inviteExamOfficerSchema = z.object({
  email:     z.string().email('Valid email required'),
  full_name: z.string().min(2, 'Full name required'),
})

// Bootstraps a university's first Exam Officer. There is no equivalent path
// through inviteUser() below: /admin/* (where that action's UI lives) is
// gated to school_admin, so a super_admin can never reach it — without this,
// a newly created university has no one able to invite lecturers or set up
// courses at all.
export async function inviteExamOfficer(universityId, prevState, formData) {
  await requireRole('super_admin')
  ...
  const parsed = inviteExamOfficerSchema.safeParse(raw)
  if (!parsed.success) return { errors: parsed.error.flatten().fieldErrors }
...
export async function updateUniversityBranding(universityId, prevState, formData) {
  ...
  if (!data || data.length === 0) return { errors: { _form: 'University not found.' } }
