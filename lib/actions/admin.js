...
export async function updateInstitutionSettings(prevState, formData) {
  ...
  // Deliberately not revalidating the whole /admin layout here — that
  // remounts every client component under it (including this very form),
  // wiping the just-set useActionState success flag before the admin ever
  // sees "Saved." The sidebar/login branding picks up the change on its
  // own next navigation anyway (its layout re-runs server-side per
  // request), so a page-only revalidation is enough.
