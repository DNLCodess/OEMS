...
describe('endStudentSession', () => {
  ...
  it('redirects to a slug-scoped /check-result/{slug} returnTo, so the university-branded path is preserved', async () => {
    const client = createMockSupabaseClient()
    client.auth.signOut.mockResolvedValue({ error: null })
    client.auth.getUser.mockResolvedValue({ data: { user: { id: 'stu-1' } } })
    createClient.mockResolvedValue(client)
    ...
    await expect(endStudentSession(null, '/check-result/pcu')).rejects.toThrow('REDIRECT')

    expect(redirect).toHaveBeenCalledWith('/check-result/pcu')
  })

  it('rejects a returnTo that merely starts with /check-result but is not slug-shaped (e.g. a lookalike host)', async () => {
    const client = createMockSupabaseClient()
    client.auth.signOut.mockResolvedValue({ error: null })
    client.auth.getUser.mockResolvedValue({ data: { user: { id: 'stu-1' } } })
    createClient.mockResolvedValue(client)
    ...
    await expect(endStudentSession(null, '/check-result-evil.com')).rejects.toThrow('REDIRECT')
