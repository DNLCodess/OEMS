...
describe('startExam', () => {
  ...
  it('auto-submits and returns an error when resuming an attempt past its deadline', async () => {
    ...
    // from() call order: [0] startExam's exam fetch, [1] startExam's existing-attempt
    // lookup, [2] submitExam's own attempt fetch, [3] exam_questions, [4] responses,
    // [5] submitExam's pass_mark fetch, [6] submitExam's attempts.update call.
    const attemptsBuilder = supabase.from.mock.results[6].value // submitExam's attempts.update call
...
describe('saveAnswer', () => {
  ...
  it('rejects a save once the attempt is past its deadline plus grace period', async () => {
    const startedAt = new Date(Date.now() - 62 * 60 * 1000).toISOString() // 62 min ago
    const supabase = createMockSupabaseClient({
      attempts: [
