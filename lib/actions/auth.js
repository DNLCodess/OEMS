...
export async function signIn(prevState, formData) {
  ...
  if (universitySlug) {
    ...
    const { data: uni } = await createAdminClient()
      ...
      .maybeSingle()
    // super_admin has university_id: null by design — they're platform-wide,
    // not scoped to any one institution, so the "wrong portal" concept
    // doesn't apply to them at all. Skip the mismatch check entirely rather
    // than signing them out with a misleading "different institution" error.
    if (uni && uni.id !== profile?.university_id && profile?.role !== 'super_admin') {
