...
import { grade } from '@/lib/exam/grading'
...
export async function submitExam(attemptId) {
  ...
  // Fetch pass_mark (and access_code, needed below to revalidate the lab result page)
  const { data: exam } = await supabase
    .from('exams')
    .select('pass_mark, access_code')
  ...
  if (exam?.access_code) revalidatePath(`/lab/${exam.access_code}/result`)
