...
import { SESSION_CHANNEL_COOKIE } from '@/lib/supabase/studentSession'
...
// may take exams.
async function requireExamAccessChannel() {
  const cookieStore = await cookies()
  const channel = cookieStore.get(SESSION_CHANNEL_COOKIE)?.value
  return channel === 'exam_access'
}
...
export async function startExam(examId) {
  ...
  if (!(await requireExamAccessChannel())) return EXAM_ACCESS_REQUIRED_ERROR
