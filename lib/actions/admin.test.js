...
describe('inviteUser', () => {
  ...
  it('creates the user with a temporary password and the right metadata', async () => {
    ...
    // Password is randomly generated per invite (not a shared static
    // value — see security notes on generateTempPassword) so assert its
    // shape rather than an exact string, and read back whatever was
    // actually passed to createUser for the rest of the assertions.
    const createUserCall = adminClient.auth.admin.createUser.mock.calls[0][0]
    expect(createUserCall.password).toMatch(/^[A-Za-z0-9_-]{20,}$/)

    expect(adminClient.auth.admin.createUser).toHaveBeenCalledWith({
      email: 'lecturer@example.com',
      password: createUserCall.password,
      email_confirm: true,
      user_metadata: {
        full_name: 'Dr. New',
        role: 'lecturer',
    ...
    expect(result).toEqual({ ok: true, email: 'lecturer@example.com', tempPassword: createUserCall.password })
  })
  ...
  it('updates department and faculty after creating the user when provided', async () => {
    ...
    expect(result).toEqual({ ok: true, email: 'lecturer2@example.com', tempPassword: expect.any(String) })
  })
})
...
describe('bulkUploadStudents', () => {
  ...
  it('normalizes a lower/mixed-case pasted matric number to uppercase before storing it', async () => {
    ...
    expect(call.email).toBe('csc2021003@uni-1.students.pcu-cbt.internal')
    expect(result.createdCount).toBe(1)
  })
...
describe('createUniversity', () => {
  ...
  it('accepts a root-relative logo_url pointing at an asset already in the public folder', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient({ universities: [{ data: null, error: null }] })
    createClient.mockResolvedValue(supabase)

    const result = await createUniversity(undefined, formData({
      name: 'Precious Cornerstone University', subdomain: 'pcu', logo_url: '/pcu/pcu-logo.jpeg',
    }))
  ...
  it('rejects a logo_url that is neither a full URL nor a root-relative path', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient()
    createClient.mockResolvedValue(supabase)

    const result = await createUniversity(undefined, formData({
      name: 'University of Lagos', subdomain: 'unilag', logo_url: 'not-a-url-or-path',
    }))
  ...
  it('rejects a reserved subdomain that would collide with a static route', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient()
    createClient.mockResolvedValue(supabase)

    const result = await createUniversity(undefined, formData({ name: 'Lab University', subdomain: 'lab' }))

    expect(result.errors.subdomain).toBeDefined()
    expect(supabase.from).not.toHaveBeenCalledWith('universities')
  })

  it('accepts a normal, non-reserved subdomain', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient({ universities: [{ data: null, error: null }] })
    createClient.mockResolvedValue(supabase)
...
describe('updateUniversityBranding', () => {
  ...
  it('updates the color and logo for the given university only', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient({ universities: [{ data: [{ id: 'uni-1' }], error: null }] })
    createClient.mockResolvedValue(supabase)
  ...
  it('returns a not-found error when the university id matches no row, instead of a false ok', async () => {
    requireRole.mockResolvedValue(superAdmin)
    const supabase = createMockSupabaseClient({ universities: [{ data: [], error: null }] })
    createClient.mockResolvedValue(supabase)

    const result = await updateUniversityBranding('nonexistent-uni', undefined, formData({
      primary_color: '#123456', logo_url: '',
    }))

    expect(result).toEqual({ errors: { _form: 'University not found.' } })
  })
})
