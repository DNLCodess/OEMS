...
export async function mintStudentSession(email, channel, examId = null) {
  ...
  if (error || !data?.properties?.hashed_token) {
    console.error('[mintStudentSession] generateLink failed', error?.message)
    return GENERIC_SESSION_ERROR
  }

  const appMetadata = { session_channel: channel, verified_exam_id: examId ?? null }
  ...
  if (metadataError) {
    console.error('[mintStudentSession] updateUserById failed', metadataError.message)
    return GENERIC_SESSION_ERROR
  }
  ...
  if (verifyError) {
    console.error('[mintStudentSession] verifyOtp failed', verifyError.message)
    return GENERIC_SESSION_ERROR
  }
