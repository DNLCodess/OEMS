...
`exams/[id]/page.js` and `exams/[id]/results/page.js` special case: both have the same line, `if (!exam || exam.created_by !== user.id) notFound()`, which currently conflates "query failed" with "no such row" — both funnel into a 404, which is misleading when the exam does exist but the query genuinely failed. This gets split in both files: a real `error` on the primary `exam` query renders the page-level banner instead of the question builder/sidebar (or the results table) — the page has no meaningful shell without `exam`, so the banner is the only content shown; `!exam` with no `error` still calls `notFound()` (→ proper 404).
