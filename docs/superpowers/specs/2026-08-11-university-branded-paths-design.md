...
- `universities.subdomain` (`TEXT NOT NULL UNIQUE`, `^[a-z0-9-]+$`) is already collected at creation and displayed as decorative text (`pcu.pcu-cbt.edu.ng`) in the super-admin's Universities screens — nothing in the app currently reads the request's hostname or path to resolve a tenant.
...
Logo: `Sidebar.js`'s existing brand mark (a `GraduationCap` icon in a colored square, next to the "PCU CBT" wordmark) shows the university's `logo_url` image in place of the icon when one is set, falling back to the current icon otherwise — same "only if set" pattern as the color.
